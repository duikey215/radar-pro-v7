...
def analyze_page(url):
    try:
        time.sleep(random.uniform(0.1, 0.2)) # Optimized Stealth
...
def audit(url: str = Query(...)):
    ...
    # 1. URL Formatting (Ensure it has protocol)
    if not url.startswith(("http://", "https://")):
        url = "https://" + url

    try:
        # 2. Fetch Homepage & Get EXACT Final URL (Solves HTTP/HTTPS Duplicate Bug)
        home_res = requests.get(url, headers=HEADERS, timeout=12, allow_redirects=True)
        final_url = str(home_res.url) # Gets the actual loaded URL (e.g., https://www.site.com/)
        ...
        # 3. Accurate Link Extraction based on Final Domain
        ...
        # 6. SCORING & ADVICE ENGINE
        score = 100
        advice =[]
        # Unique keys are mapped to advice for frontend "Fix Now" links
        ...
        return JSONResponse({"error": "Failed to connect to the target website. Check the URL or ensure the site allows bots."}, status_code=500)from fastapi import FastAPI, Query
from fastapi.responses import JSONResponse
import requests
from bs4 import BeautifulSoup
from urllib.parse import urljoin, urlparse
